    ...
    serialPort(new QSerialPort)
{
    ...
    connect(readTimer, &QTimer::timeout, this, &YmodemFileReceive::readTimeOut);
    connect(writeTimer, &QTimer::timeout, this, &YmodemFileReceive::writeTimeOut);
}
...
Ymodem::Code YmodemFileReceive::callback(Status status, uint8_t *buff, uint32_t *len)
{
    switch(status)
    {
        case StatusEstablish:
        {
            if(buff[0] != 0)
            {
                ...
                if(file->open(QFile::WriteOnly) == true)
                {
                    ...
                    emit receiveStatus(StatusEstablish);
        ...
        case StatusTransmit:
        {
            ...
            emit receiveStatus(StatusTransmit);
